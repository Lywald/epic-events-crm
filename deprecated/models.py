...
class Client(Base):
    ...
    commercial    = relationship("User", back_populates="clients", foreign_keys=[commercial_id])
...
class Contract(Base):
    ...
    commercial_id    = Column(Integer, ForeignKey("users.id"), nullable=True)
    commercial       = relationship("User", back_populates="contracts", foreign_keys=[commercial_id])
